...
public interface AgendamentoRepository extends JpaRepository<Agendamento, Long> {
	...
	List<Horario> findMedicoDisponivelNaData(Long id, LocalDate data);
	...
	@Query("select a.id as id, a.paciente as paciente,"
		...
		+ "from Agendamento a where a.paciente.usuario.email like :email")
	Page<HistoricoPaciente> findHistoricoByPacienteEmail(String email, Pageable pageable);

	@Query("select a.id as id, a.paciente as paciente,"
			+ "CONCAT(a.dataConsulta, ' ', a.horario.horaMinuto) as dataConsulta,"
			+ "a.medico as medico, a.especialidade as especialidade "
			+ "from Agendamento a where a.medico.usuario.email like :email")
